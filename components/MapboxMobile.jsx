...
const MapboxMobile = ({ isInteractive = true }) => {
  ...
  const geolocationAccepted = useMapStore((s) => s.geolocationAccepted);
  ...
  const updateClusters = useCallback(() => {
    ...
    if (eventsSuperclusterRef.current) {
      const clusters = eventsSuperclusterRef.current.getClusters(
        boundsArray,
        zoomLevel,
      );
    ...
    if (orlikSuperclusterRef.current && showMarkers) {
      const clusters = orlikSuperclusterRef.current.getClusters(
        boundsArray,
        zoomLevel,
      );
  ...
  const handleEventMarkerPress = useCallback((featureId) => {
    ...
    if (isCluster && eventsSuperclusterRef.current) {
      const leaves = eventsSuperclusterRef.current.getLeaves(
        cluster_id,
        Infinity,
      );
  ...
  const handleOrlikMarkerPress = useCallback((featureId) => {
    ...
    if (isCluster && orlikSuperclusterRef.current) {
      const leaves = orlikSuperclusterRef.current.getLeaves(
        cluster_id,
        Infinity,
      );
  ...
  const handleMapLoad = useCallback(() => {
    ...
    mapRegionRef.current = {
      bounds: {
        ne: [
          camera.centerCoordinate[0] + 0.5,
          camera.centerCoordinate[1] + 0.5,
        ],
        sw: [
          camera.centerCoordinate[0] - 0.5,
          camera.centerCoordinate[1] - 0.5,
