...
const ChatRoom = () => {
    ...
    const resolveRoom = async () => {
      ...
      try {
        if (paramRoomId) {
          ...
          const existing = useSocketStore
            .getState()
            .roomsState.find((r) => r.roomId === paramRoomId);
          if (existing) {
            setRoom(existing);
          } else {
            ...
            if (found) {
              ...
              Alert.alert(
                "Info",
                "Pokój czatu nie istnieje lub został usunięty",
              );
              router.back();
              return;
            }
          ...
          const roomExists = useSocketStore
            .getState()
            .roomsState.some((r) => r.roomId === newRoom.roomId);
          if (!roomExists) {
            addRoom(newRoom);
  ...
  // Gap sync — po odzyskaniu połączenia (powrót z tła / reconnect) dociągnij
  // wiadomości pominięte gdy socket był rozłączony. Cicho — bez spinnera,
  // merge po _id, więc brak duplikatów i brak skoku UI.
  const prevConnectedRef = useRef(isConnected);
  useEffect(() => {
    const wasDisconnected = prevConnectedRef.current === false;
    prevConnectedRef.current = isConnected;
    if (!isConnected || !wasDisconnected || !room) return;
    ...
    const syncMissedMessages = async () => {
      try {
        ...
        setMessages((prev) => {
          ...
          const missed = latest.filter((m) => !known.has(m._id));
          // missed są najnowsze (newest-first) — prepend do inverted listy
          return missed.length > 0 ? [...missed, ...prev] : prev;
        });
        customFetch
          .patch("/chat/messages/read", { roomId: room.roomId })
          .catch(() => {});
        setRoomAsRead(room.roomId);
      } catch (error) {
        console.error("[ChatRoom] Gap sync failed:", error?.message);
      }
    };
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (msg) => {
      ...
      if (currentRoom && currentRoom.roomId === msg.roomId) {
        ...
        customFetch
          .patch("/chat/messages/read", { roomId: currentRoom.roomId })
          .catch(() => {});
  ...
  const handleMuteChat = async (duration) => {
    ...
    if (duration === "1h") muteExpiresAt = new Date(Date.now() + 3600000);
      ...
      muteExpiresAt = new Date(Date.now() + 43200000);
    else if (duration === "24h")
      muteExpiresAt = new Date(Date.now() + 86400000);
    else if (duration === "1w")
      muteExpiresAt = new Date(Date.now() + 604800000);
  ...
  const roomName = useMemo(() => {
    ...
    if (room.roomType === "group")
      return room.eventName || "Wydarzenie grupowe";
  ...
  const keyboardPadding = useAnimatedStyle(
    () => ({
      height:
        height.value > 0 ? Math.max(0, height.value - insetsBottomSV.value) : 0,
    }),
    [],
  );
