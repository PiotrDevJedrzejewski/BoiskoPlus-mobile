...
export const NotificationProvider = ({ children }) => {
  ...
  //
  // UWAGA routing: ścieżki muszą istnieć w app/(auth)/ (v2!):
  //   chat → /(auth)/chat-room?roomId=...   (NIE /(auth)/chat/[roomId])
  //   eventy/notifications → events-dashboard (ekran /notifications nie istnieje)
  // Provider montuje się dopiero wewnątrz AuthLayout (po auth-guard),
  // więc nawigacja nigdy nie trafia na niezalogowanego usera.
  // ───────────────────────────────────────────────
  const resolveNotificationRoute = useCallback((data) => {
    ...
    if (data?.screen === "friends" || data?.type === "friendRequest") {
      return { pathname: "/(auth)/friends" };
    }
    if (
      data?.type === "eventStatusUpdate" ||
      data?.type === "eventReminder" ||
      data?.screen === "notifications"
    ) {
      // Z eventId prowadzimy prosto do szczegółów wydarzenia
      if (data?.eventId) {
        return {
          pathname: "/(auth)/single-event",
          params: { id: data.eventId },
        };
      }
      return { pathname: "/(auth)/events-managment/events-dashboard" };
    }
    return null;
  }, []);
  ...
  const navigateFromNotification = useCallback(
    (response) => {
      if (!response) return;
      // Dedupe — cold-start potrafi dostarczyć tę samą odpowiedź
      // przez listener ORAZ getLastNotificationResponseAsync
      const id = response.notification?.request?.identifier;
      if (id && handledNotificationIdRef.current === id) return;
      ...
      // Wejście z pusha "konsumuje" powiadomienie in-app dla tego eventu
      if (data?.eventId) {
        const store = useSocketStore.getState();
        store.markEventAsRead(data.eventId).catch(() => {});
        store.clearInvite(data.eventId);
      }

      // Odrocz nawigację — przy cold starcie Stack mógł się jeszcze
      // nie zamontować (push do niezaładowanej trasy → 404/pusty ekran)
      setTimeout(() => {
        try {
          router.push(route);
        } catch (error) {
    ...
    [router, resolveNotificationRoute],
  );

  useEffect(() => {
    // Handler dla tapu w powiadomienie (app w tle)
    notificationResponseListener.current =
      Notifications.addNotificationResponseReceivedListener(
        navigateFromNotification,
      );

    // Cold start — app została URUCHOMIONA przez tap w push.
    // Listener wyżej nie odpala się dla tej odpowiedzi — trzeba ją pobrać ręcznie.
    // Provider montuje się w AuthLayout, więc auth jest już rozstrzygnięty.
  ...
  const shouldShowNotification = useCallback(
    (type, chatRoomID = null, eventId = null) => {
